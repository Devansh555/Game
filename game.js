...
function start() {
    ...
    //Created another event listener to take the input of the speed of the bear
    ...
    //calling to update to move the bees
    updateBees()
...
//Function which generates random number from 0 to max
...
function moveBees() {
    ...
    for (let i = 0; i < bees.length; i++) {
        ...
        bees[i].move(dx, dy);
        isHit(bees[i], bear); //Adding this line to count the number of stings
    }
}
function updateBees() { // update loop for game
    ...
    let period = document.getElementById("periodTimer").value; //Using the input field periodTimer instead of the fixed period
    //update the timer for the next move
    if (hits.innerHTML>=1000){  //If the hits have reached 1000
        alert("GAME OVER!");    //The game stops with the message GAME OVER!
    }
    else{
